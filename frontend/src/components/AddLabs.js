...
export default function AddLabs({ onSuccess, onClose, isEditMode, editData }) {
  ...
  useEffect(() => {
    if (isEditMode && editData) {
      setForm(editData); // pre-fill form with existing data
    }
  }, [isEditMode, editData]);
  ...
  const handleSubmit = () => {
    if (!form.category || !form.managerName) {
              ...
              <Dialog.Panel className="relative transform overflow-hidden rounded-lg bg-white text-left shadow-xl transition-all sm:w-full sm:max-w-lg">
                <div className="bg-white px-4 py-5 sm:p-6">
                  ...
                  <div className="mt-6 flex justify-end gap-3">
                    ...
                    >
                      {loading ? "Processing..." : isEditMode ? "Update" : "Add Lab"}
                    </button>
                    <button
                      type="button"
                      onClick={onClose}
